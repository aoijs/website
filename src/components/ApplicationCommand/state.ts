...
interface StateMethods {
    update<K extends keyof ApplicationCommand>(key: K, val?: ApplicationCommand[K]): void;
...
//@ts-expect-error Invalid types from package
export const useCommandStore = create<Partial<ApplicationCommand> & StateMethods>((set, get) => ({
    update: (key, val) => {
    ...
    updateOption: (option) => {
        set((state) => ({
            ...state,
            options: [...state.options!.filter((opt) => opt.key !== option.key), option]
        }));
    },
    getCleanedCommandJSON: () => {
        ...
        return typeof mutableCommand.options === "object" ? JSON.stringify(mutableCommand.options, null, 2) : null;
    }
}));
